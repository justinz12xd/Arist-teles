...
VALUES ('analysis:%', 'Private Aristóteles analysis progress', true)
ON CONFLICT (pattern) DO UPDATE SET description = EXCLUDED.description, enabled = EXCLUDED.enabled;

CREATE OR REPLACE FUNCTION public.publish_analysis_progress()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status OR OLD.progress IS DISTINCT FROM NEW.progress OR OLD.current_stage IS DISTINCT FROM NEW.current_stage THEN
...
CREATE TRIGGER analysis_progress_trigger
AFTER UPDATE ON public.analysis_runs
