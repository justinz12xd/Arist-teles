...
const STACK = ["NEXT", "FASTAPI", "DEEP AGENTS", "OPENROUTER", "INSFORGE", "PGVECTOR"];
const INPUTS = ["PDF DIGITAL", "PDF ESCANEADO", "PNG", "JPEG", "WEBP", "REPORTE JSON"];
    ...
    <section id="partners" className="relative mx-auto max-w-6xl scroll-mt-[var(--header-h)] px-6 py-32">
      <ScrollReveal>
        ...
        <h2 className="text-h2 max-w-3xl">
          Frontend de decision, backend asincrono y recuperacion semantica aislada por usuario.
        </h2>
      </ScrollReveal>

      <div className="mt-16 grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3 md:grid-cols-6">
        {[...STACK, ...INPUTS].map((item, i) => (
